...
import CV_Intel.calculate_rmsd_kabsch as rmsd
import numpy as np
from CV_Intel.helper_functions import cv_find_chessboard, get_chessboard_points_3D, get_depth_at_pixel, convert_depth_pixel_to_metric_coordinate
...
def calculate_transformation_kabsch(src_points, dst_points):
    ...
    translation_vector = rmsd.centroid(
        dst_points) - np.matmul(rmsd.centroid(src_points), rotation_matrix)
...
class Transformation:
    ...
    def apply_transformation(self, points):
        ...
        points_transformed = np.true_divide(
            points_trans_[:3, :], points_trans_[[-1], :])
...
class PoseEstimation:
        ...
        for (serial, [found_corners, points2D, points3D, validPoints]) in corners3D.items():
            ...
            if found_corners == True:
                ...
                if valid_object_points.shape[1] < 5:
                    ...
                    retval[serial] = [True, Transformation(
                        rotation_matrix, translation_vector), points2D, rmsd_value]
                    print("RMS error for calibration with device number",
                          serial, "is :", rmsd_value, "m")
        return retval
    ...
    def find_chessboard_boundary_for_depth_image(self):
        ...
        for (serial, frameset) in self.frames.items():
            ...
            found_corners, points2D = cv_find_chessboard(
                depth_frame, infrared_frame, self.chessboard_params)
            boundary[serial] = [np.floor(np.amin(points2D[0, :])).astype(int), np.floor(np.amax(points2D[0, :])).astype(
                int), np.floor(np.amin(points2D[1, :])).astype(int), np.floor(np.amax(points2D[1, :])).astype(int)]
